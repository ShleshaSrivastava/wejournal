...
export const Navigation = () => {
  ...
  const resetWidth = () => {
    if (sidebarRef.current && navbarRef.current) {
      ...
      navbarRef.current.style.setProperty("left", isMobile ? "100%" : "310px");
      ...
      >
        ...
        <div className="flex items-center">
          <div className="rounded-full m-4 flex overflow-hidden h-8 w-8">
            <Avatar>
              <AvatarImage
                src={user?.photoURL ? user.photoURL : "/reading.png"}
              />
            </Avatar>
          </div>
        ...
        <nav className="bg-transparent px-3 py-2 w-full">
          {isCollapsed && (
            <MenuIcon
              onClick={resetWidth}
              role="button"
              className="h-8 w-8 text-muted-foreground bg-white p-2 rounded-full border"
            />
          )}
        </nav>
      </div>
